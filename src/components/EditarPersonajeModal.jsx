...
export default function EditarPersonajeModal({ personajeId, nombre, onClose, onChanged }) {
  ...
  // Instancia que vino del origen o del background: no se puede quitar, porque
  // el personaje no eligió tenerla y no habría cómo reclamarla de nuevo. El
  // backend la rechaza con 409; aquí solo se evita ofrecer el botón.
  // Si el feat es repetible y hay varias filas, la protegida es la más antigua
  // (extra_feats viene ordenado por personaje_feat_id), que es la de la creación.
  const featCreacionId = (() => {
    const otorgados = [full?.origin_feat?.feat_id, full?.background_feat?.feat_id]
      .filter(Boolean).map(Number)
    const ids = new Set()
    for (const f of extraFeats) {
      const i = otorgados.indexOf(Number(f.feat_id))
      if (i === -1) continue
      otorgados.splice(i, 1)
      ids.add(Number(f.personaje_feat_id))
    }
    return ids
  })()
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
            ...
            <div>
              ...
              {extraFeats.length === 0 ? (
                ...
                <div className="space-y-1.5">
                  {extraFeats.map((f, i) => {
                    const deCreacion = featCreacionId.has(Number(f.personaje_feat_id))
                    return (
                    <div key={f.personaje_feat_id} className="flex items-center justify-between gap-2 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                      <div className="flex items-center gap-1.5 min-w-0 flex-wrap">
                        ...
                        {deCreacion && (
                          <span className="text-[10px] font-bold text-gray-600 bg-gray-200 border border-gray-300 rounded px-1.5 py-0.5">
                            de la creación
                          </span>
                        )}
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button onClick={() => { setErrorDel(''); setConfirmDel(f) }} disabled={deCreacion}
                          title={deCreacion ? 'Lo otorgan el origen o el background: no se puede quitar' : 'Eliminar rasgo'}
                          className="text-gray-400 hover:text-red-600 disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:text-gray-400">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </div>
